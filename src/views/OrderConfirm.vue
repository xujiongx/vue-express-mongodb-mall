      ...
      <div class="checkout-order">
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <router-link href="javascript:void(0)" class="btn btn--m" to="/address">Previous</router-link>
          </div>
...
export default {
  ...
  methods: {
    ...
    //订单支付事件
    payMent(){
        const addressId=this.$route.query.seletedAddrId;
        axios.post('/users/payMent',{
            addressId:addressId,
            orderTotal:this.orderTotal
        }).then(res=>{
            if(res.data.status==1){
                console.log('支付成功'+res.data.result.orderTotal)
                this.$router.push({
